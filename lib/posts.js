...
function getRecordById(collection, id, fields) {
  ...
  if (fields == null) {
    Object.assign(recordObject, data);
  } else {
...
function getRecords(collection, fields) {
  return getAllFilenames(collection).map((filename) =>
    getRecordById(collection, filename, fields)
  );
}
...
export function getEvents(fields) {
  return getRecords("events", fields).sort((a, b) =>
...
export function getTeam(wantedTeam, fields) {
  return getRecords("members", fields).filter(
    ({ team }) => team === wantedTeam
  );
}
