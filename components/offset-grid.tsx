...
export default function OffsetGrid({ elements }: Props) {
  ...
  return (
    <div className="mt-6 mb-14 grid grid-cols-1 grid-rows-4 gap-8 md:grid-cols-2 md:grid-rows-2 md:mb-40 md:gap-16">
      {elements.map(({ heading, text, icon }, index) => {
        const bgColor = bgColors[index % bgColors.length];
        return (
          <div
            key={index}
            className="div-style1 relative flex w-96 flex-col justify-center overflow-hidden px-9 py-6 md:even:top-1/2 md:w-80 md:px-12 md:py-8 lg:w-96"
          >
